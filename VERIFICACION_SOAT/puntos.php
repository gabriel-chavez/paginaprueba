...
<head>
  ...
  <?php
            ...
            if($_POST['ct_ciudad'] != ''){
              ...
              $direcciones = '';
              $query = 'SELECT *, (SELECT entidad FROM soat_entidad WHERE id_entidad = d.T_PAR_BANCOS_FK) AS entidad, (SELECT codigo FROM t_par_catalogo WHERE id = d.T_PAR_CATALOGO) AS codigo
              ...
              if ($res_d->num_rows > 0) {
                  while($rw_d = $res_d->fetch_assoc()) {
                    ...
                    $direcciones.= '{position: new google.maps.LatLng('.$rw_d["LATITUD"].', '.$rw_d["LONGITUD"].'),type: "'.$rw_d["codigo"].'",contentString: "<strong><h3 style=\"font-size:14px\">'.$rw_d["entidad"].'</h3></strong><strong>AGENCIA:</strong> '.$rw_d["AGENCIA"].'<br><br><strong>DIRECCIÓN:</strong> '.$rw_d["ID"].'-'.comillas(utf8_encode($rw_d["DIRECCION"])).comillas(utf8_encode($datos_adicionales)).'"}, 
                    ';
                  }
              }
            }
  ...
  <script>
          ...
          function initMap() {
            ...
            var iconBase = 'http://www.univida.bo/images/';
            var icons = {
              univida: {
        ...
        <script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCyiYSjXh0hkzcXTf0U4vKn2QkCN8ItDmE&callback=initMap"></script>
  <style>
