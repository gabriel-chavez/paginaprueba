...
function sb_get_content_sections($parent_id = 0)
{
	$parent_id = (int)$parent_id;
	$sections = LT_HelperContent::GetSections($parent_id);
	foreach($sections as $index => $s)
	{
		$sections[$index]->childs = sb_get_content_sections($s->section_id);
	}
...
function sb_sections_dropdown($args)
{
	sb_include_module_helper('content');
	$args = array_merge(array('id' => 'section_id', 'option_text' => __('-- seccion --', 'content'), 'selected' => -1), $args);
...
{
	sb_include_module_helper('content');
	$args = array_merge(array('id' => 'sections', 'checked' => array(), 'show_checkbox' => true, 'show_links' => false, 'class' => 'list'), $args);
	...
	function ___get_section_list_childs($_s, $_args)
	{
		...
		foreach($_s->childs as $__s)
		{
			...
			if( $_args['show_checkbox'] )
				$ops .= '<input type="checkbox" name="section[]" value="'.$__s->section_id.'" '.(in_array($__s->section_id, $_args['checked']) ? 'checked' : '').' />';
			if( $_args['show_links'] )
			{
	...
	foreach($sections as $s)
	{
		...
		if( $args['show_checkbox'] )
			$select .= '<input type="checkbox" name="section[]" value="'.$s->section_id.'" '.(in_array($s->section_id, $args['checked']) ? 'checked' : '').' />';
		if( $args['show_links'] )
		{
...
{
	sb_include_module_helper('content');
	$args = array_merge(array('id' => 'categories', 'checked' => array(), 'show_checkbox' => true, 'show_links' => false, 'class' => 'list'), $args);
