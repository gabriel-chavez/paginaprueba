...
<div class="wrap">
	...
	<tbody>
	<?php $i = 1; foreach($obj->GetApplicants(null, SB_Request::getString('keyword'), SB_Request::getString('search_by')) as $a): ?>
	<tr>
		...
		<td>
			<a href="<?php print SB_Route::_('index.php?mod=rrhh&view=announcements.view_profile&id='.$a->id.'&bid='.$a->bid); ?>" class="btn btn-default btn-sm" 
				target="_blank" title="<?php _e('View Profile', 'rrhh'); ?>">
				<span class="glyphicon glyphicon-eye-open"></span>
			</a>
			<a href="javascript:;" class="btn btn-default btn-sm btn-cf"
				title="<?php _e('See Curricular Filter', 'rrhh'); ?>"
				data-id="<?php print $a->id; ?>">
				<span class="glyphicon glyphicon-list-alt"></span>
			</a>
			<!-- <a href="<?php print SB_Route::_('index.php?mod=rrhh&task=announcements.remove_bid&id='.$obj->id.'&bid='.$a->bid); ?>" 
				class="btn btn-default btn-sm confirm" 
				data-message="<?php _e('Are you sure to remove the applicant?', 'rrhh'); ?>" 
				title="<?php _e('Remove applicant', 'rrhh'); ?>">
...
jQuery(function()
{
	...
	jQuery(document).on('click', '.btn-cf', function()
	{
		CurricularFilter(this.dataset.id);
		return false;
	});
});
function CurricularFilter(item)
{
	var fields = jQuery('#hidden-applicants');
	fields.html('');
	if( !item )
	{
		var ids = jQuery('.tcb-select:checked');
		if( ids.length <= 0 )
		{
		...
		jQuery.each(ids, function(i, input)
		{
			fields.append('<input type="hidden" name="person_id[]" value="'+input.value+'" />');
		});
	}
	else
	{
		fields.append('<input type="hidden" name="person_id[]" value="'+item+'" />');
	}
...
<form id="hidden-form" action="" method="post">
	<input type="hidden" name="mod" value="rrhh" />
	<input type="hidden" name="task" value="announcements.see_curricular_filter" />
	<input type="hidden" name="id" value="<?php print $obj->id; ?>" />
	<div id="hidden-applicants"></div>
</form>