...
class LT_AdminControllerRrhhAnnouncements extends SB_Controller
{
	...
	{
		SB_Module::add_action('table_list_before_show_item', 'rrhh_review_announcement_status');
		...
		$table->SetColumns(array(
				...
				'start_date'			=> array('label' => __('Start Date', 'rrhh'), 'callback' => 'sb_format_date'),
				...
				'creation_date'			=> array('label' => __('Creation Date', 'rrhh'), 'callback' => 'sb_format_datetime'),
		));
		...
		$table->SetRowActions(array(
				...
				'task:announcements.delete'		=> array('label' => __('Delete', 'rrhh'), 
															'icon' => 'glyphicon glyphicon-trash', 
															'class' => 'confirm',
															'data' => array('message' => 'Esta seguro de borrar la convocatoria?') )
		));
		/*
	...
	public function task_delete()
	{
		...
		$this->dbh->Delete('rrhh_announcements', array('id' => $id));
	...
	public function task_see_curricular_filter()
	{
		...
		$sheet->setCellValue("b1", $obj->code);
		$sheet->setCellValue("c1", 'Convocatoria');
		$sheet->setCellValue("d1", $obj->name);
		$row = 5;
		foreach($obj->GetApplicants($ids) as $a)
		{
			...
			$sheet->setCellValue("A$row", 'Postulante');
			$sheet->mergeCells("A$row:G$row");
			$sheet->getStyle("A$row")->applyFromArray(
					array(
							'fill' => array(
									'type' => PHPExcel_Style_Fill::FILL_SOLID,
									'color' => array('rgb' => '4662b1')
							),
							'font'  => array(
									'bold'  => true,
									'color' => array('rgb' => 'FAE816'),
									'size'  => 10,
									//'name'  => 'Verdana'
							),
							'alignment' => array(
			...
			foreach($records as $rec)
			{
				...
				$sheet->setCellValue("e$row", isset($data->academic) ? $data->academic->{'id_'.$rec->id}->points : 0);
				$row++;
			}
			...
			foreach($data->general_experience as $exp)
			{
				...
				$sheet->setCellValue("e$row", (int)$exp->currently_working == 1 ? sb_format_date($exp->fend_date) : sb_format_date($exp->end_date));
	...
	public function task_remove_bid()
	{
		$id	 = SB_Request::getInt('id');
		$bid = SB_Request::getInt('bid');
		$this->dbh->Delete('rrhh_announcement2person', array('id' => $bid));
		SB_MessagesStack::AddMessage(__('The applicant has been removed', 'rrhh'), 'success');
		sb_redirect(SB_Route::_('index.php?mod=rrhh&view=announcements.applicants&id='.$id));
	}
	public function task_close()
	{
		set_time_limit(0);
		$id 			= SB_Request::getInt('id');
		$person_id 		= (array)SB_Request::getVar('person_id', array());
		$message_tpl	= SB_Request::getString('message');
		//##get announcement
		$obj = new RRHH_Announcement($id);
		
		$this->dbh->Update('rrhh_announcements', array('status' => 'closed', 
														'selected' => json_encode($person_id)
												), 
												array('id' => $id)
		);
		$subject 		= "Convocatoria Cerrada - $obj->name";
		$email_from 	= defined('RRHH_EMAIL_FROM') ? RRHH_EMAIL_FROM : 'no-reply@littlecms.com';
		$headers 		= array(
			'Content-type: text/html',
			sprintf("From: %s<%s>", SITE_TITLE, $email_from)
		);
		lt_mail('marce_nickya@yahoo.es', $subject, $message_tpl, $headers);
		//##get un selected persons
		foreach($obj->GetApplicants() as $person)
		{
			if( !in_array($person->id, $person_id) )
			{
				//##send message to unselected person
				$message = $message_tpl;
				lt_mail($person->email, $subject, $message, $headers);
			}
