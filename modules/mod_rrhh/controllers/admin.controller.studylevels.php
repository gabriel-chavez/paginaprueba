...
class LT_AdminControllerRrhhStudylevels extends SB_Controller
{
	public function task_default()
	{
		...
		$table->SetRowActions(array(
				...
				'task:studylevels.delete'	=> array('label' => __('Edit', 'rrhh'), 'icon' => 'glyphicon glyphicon-trash', 
														'class' => 'confirm', 'data' => array('message' => __('Are you sure to delete the study level?', 'rrhh')))
		));
	...
	public function task_delete()
	{
		...
		if( !$id )
		{
			SB_MessagesStack::AddMessage(__('The study level identifier is invalid', 'rrhh'), 'error');
			sb_redirect(SB_Route::_('index.php?mod=rrhh&view=studylevels.default'));
		}
		$this->dbh->Delete('rrhh_study_levels', array('id' => $id));
		SB_MessagesStack::AddMessage(__('The study level has been deleted', 'rrhh'), 'success');
		sb_redirect(SB_Route::_('index.php?mod=rrhh&view=studylevels.default'));
	}
