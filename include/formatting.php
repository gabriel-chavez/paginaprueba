 ...
 *
 * Specifically, HTML and PHP tags are stripped. If $title is empty and $fallback_title is set, the latter
 * will be used.
 *
 *
 ...
 */
function sb_sanitize_title( $title, $fallback_title = '', $context = 'save' ) {
	$raw_title = $title;

	if ( 'save' == $context )
		$title = sb_remove_accents($title);
 ...
 */
function sb_remove_accents($string) 
{
	if ( !preg_match('/[\x80-\xff]/', $string) )
		return $string;

	if (sb_seems_utf8($string)) 
	{
		$chars = array(
...
function sb_build_slug($str)
{
	$text = sb_sanitize_title($str);
	
	return preg_replace('/[^a-zA-Z0-9]/', '-', strtolower($text));
}
function sb_get_unique_slug()
{
