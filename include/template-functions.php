...
function lt_favicon()
{
	print '<link rel="icon" type="image/png" href="'. SB_Module::do_action('favicon', TEMPLATE_URL.'/images/favicon.png').'">';
}
function lt_head()
{
	lt_favicon();
...
function lt_add_tinymce($args = null)
{
	...
	$def_args = array(
			...
			'toolbar1'		=> 'insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image | youtube',
...
function lt_include_view($mod, $view, $data = array())
{
	...
	if( !file_exists($view_dir . SB_DS . $view) )
		return null;
	extract($data);
	include $view_dir . SB_DS . $view;
}
