...
function sb_insert_user($data)
{
	...
	if( !isset($data['user_id']) || !$data['user_id'] )
	{
		...
		$body = sprintf(__("Hello %s<br/><br/>", 'users'), $user->username) .
				sprintf(__('Thanks for register into our website, you account details are below.<br/><br/>')).
				sprintf(__('Username: %s<br/>', 'users'), $user->username).
				sprintf(__('Password: %s<br/>', 'users'), $pass).
				'<br/>'.
				__('Follow the next link in order to start a session.<br/><br/>').
