...
if( SB_Request::getString('send_form') )
{
	...
	// agregar de donde se envia
	$maila -> SetFrom($cemail);
	$maila -> AddReplyTo($cemail);
	...
	if ($maila -> send())
	{
		SB_MessagesStack::AddMessage(__('Su mensaje fue enviado correctamenet, le responderemos oportunamente. Gracias.'),'success');
	}
	else
	{
		SB_MessagesStack::AddMessage(__('Su mensaje ha sufrido algun error, espere e intente enviar nuevamente. Gracias.'),'success');
	}

	//lt_mail($email . ',info@sinticbolivia.net', 'Formulario de Contacto - ' . SITE_TITLE, $body, $headers);
